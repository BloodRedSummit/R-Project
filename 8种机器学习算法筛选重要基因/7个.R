...
setwd("C:\\Users\\lenovo\\Desktop\\6种算法")      #设置工作目录
...
######KKNN模型
mod_kknn=train(Type ~., data = train, method = "kknn", trControl=control)
...
###KKNN模型预测结果
explainer_kknn=explain(mod_kknn, label = "kknn",
                       data = test, y = yTest,
                       predict_function = p_fun,
                       verbose = FALSE)
mp_kknn=model_performance(explainer_kknn)
...
p1 <- plot(mp_rf, mp_svm, mp_xgb, mp_glm, mp_gbm, mp_kknn, mp_nnet)
print(p1)
dev.off()

#绘制四种方法的残差箱线图
pdf(file="boxplot.pdf", width=6, height=6)
p2 <- plot(mp_rf, mp_svm, mp_xgb, mp_glm, mp_gbm, mp_kknn , mp_nnet, geom = "boxplot")
print(p2)
dev.off()
...
pred6=predict(mod_kknn, newdata=test, type="prob")
...
legend('bottomright',
       c(paste0('RF: ',sprintf("%.03f",roc1$auc)),
         ...
         paste0('kknn: ',sprintf("%.03f",roc5$auc)),
         paste0('nnet: ',sprintf("%.03f",roc6$auc)),
         paste0('GBM: ',sprintf("%.03f",roc7$auc))),
       
       
       col=c("red","blue","green","yellow","orange","purple","black"), lwd=2, bty = 'n')
dev.off()
...
plot(importance_rf[c(1,(ncol(data)-8):(ncol(data)+1)),],
     ...
     importance_kknn[c(1,(ncol(data)-8):(ncol(data)+1)),],
...
write.table(importance_kknn[(ncol(data)-geneNum+2):(ncol(data)+1),], file="importanceGene.KKNN.txt", sep="\t", quote=F, row.names=F)
write.table(importance_kknn[(ncol(data)-geneNum+2):(ncol(data)+1),], file="importanceGene.NNET.txt", sep="\t", quote=F, row.names=F)
