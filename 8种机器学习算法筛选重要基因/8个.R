...
p1 <- plot(mp_rf, mp_svm, mp_xgb, mp_glm, mp_gbm, mp_kknn, mp_nnet,mp_lasso)
print(p1)
dev.off()

#绘制四种方法的残差箱线图
pdf(file="boxplot.pdf", width=6, height=6)
p2 <- plot(mp_rf, mp_svm, mp_xgb, mp_glm, mp_gbm, mp_kknn , mp_nnet, mp_lasso,geom = "boxplot")
print(p2)
dev.off()
...
legend('bottomright',
       c(paste0('RF: ',sprintf("%.03f",roc1$auc)),
         ...
         paste0('GBM: ',sprintf("%.03f",roc8$auc))),
       
       
       col=c("red","blue","green","yellow","orange","purple","black","red"), lwd=2, bty = 'n')
dev.off()
